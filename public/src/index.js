...
const App = () => {
  ...
  const setupWastedActionsListener = () => {
    ...
    const userWastedActionsRef = collection(db, `artifacts/<span class="math-inline">\{publicAppId\}/users/</span>{userId}/wastedActions`);
  ...
  const addWastedAction = async (actionText, duration, isAICommand, aiReason = '', aiDifficulty = '') => {
    ...
    try {
      ...
      const wastedActionRef = collection(db, `artifacts/<span class="math-inline">\{publicAppId\}/users/</span>{userId}/wastedActions`);
      ...
      setModalContent({
        title: '無駄を記録しました！',
        message: isAICommand
          ? `あなたは「<span class="math-inline">\{actionText\}」を実行し、</span>{wastePoints}無駄度ポイントを獲得しました！\n\nAIからの言葉：\n「${aiReason}」`
          : `「<span class="math-inline">\{actionText\}」を実行し、</span>{wastePoints}無駄度ポイントを獲得しました！\n\nAIからの言葉：\n「あなたのその無駄は、もはや芸術の域に達しています。しかし、まだ上があります。」`,
      });
      setShowModal(true);
